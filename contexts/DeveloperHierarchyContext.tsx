...
╚════════════════════════════════════════════════════════════════╝
  ...
  const canModify = useCallback((
    ...
    if (action === 'username' && !source.canModifyUsernames) {
      return { canModify: false, reason: 'Vous n\'avez pas la permission de modifier les pseudos' };
    }
  ...
  const attemptModification = useCallback(async (
    ...
    if (!check.canModify) {
      ...
      if (target && source && source.level > target.level) {
        const actionText = action === 'username' ? 'modifier le pseudo' : 
